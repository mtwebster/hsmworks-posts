...
var rFormat = createFormat({decimals:(unit == MM ? 3 : 4), forceSign:true});
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  if (pendingRadiusCompensation >= 0) {
    if (getCircularPlane() != PLANE_XY) {
      error("Radius compensation cannot be activated/deactivated for circular move in other plane than the XY-plane.");
    }
  }
...
function onClose() {
  ...
  if (!machineConfiguration.hasHomePositionX() && !machineConfiguration.hasHomePositionY()) {
    ...
    if (properties.format == 1) {
      ...
      writeBlock(gFormat.format(28), gAbsIncModal.format(91), gMotionModal.format(0), "X" + xyzFormat.format(0), "Y" + xyzFormat.format(0)); // return to home
    }
  } else {
    var homeX;
